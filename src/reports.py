...
def save_report(filename: Optional[str] = None) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            output_file = filename if filename else f"{func.__name__}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            try:
...
def spending_by_category(transactions: pd.DataFrame, category: str, date: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        # Определение периода (последние 3 месяца)
        start_date = end_date - timedelta(days=90)

        # Фильтрация транзакций
        transactions["Дата операции"] = pd.to_datetime(transactions["Дата операции"], format="%d.%m.%Y %H:%M:%S")
        mask = (transactions["Дата операции"] >= start_date) & (transactions["Дата операции"] <= end_date) & \
               (transactions["Категория"] == category) & (transactions["Сумма платежа"] < 0)
        filtered_transactions = transactions[mask][["Дата операции", "Сумма платежа", "Категория", "Описание"]].copy()

        # Форматирование дат
        filtered_transactions["Дата операции"] = filtered_transactions["Дата операции"].dt.strftime("%Y-%m-%d")
        filtered_transactions["Сумма платежа"] = filtered_transactions["Сумма платежа"].abs()
