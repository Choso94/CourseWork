...
def get_currency_rates() -> list:
    ...
    try:
        ...
        if not api_key:
            logger.warning("API-ключ для ExchangeRate-API не найден, возвращаются пустые курсы валют")
            return []
...
def get_stock_prices() -> list:
    ...
    try:
        ...
        if not api_key:
            logger.warning("API-ключ для Alpha Vantage не найден, возвращаются пустые цены акций")
            return []
